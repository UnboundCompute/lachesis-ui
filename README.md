...
code-property-graph — the way a developer reads a codebase, not as a raw graph.

You point it at a graph built from any Python / TypeScript / JavaScript / C
...
> **Status: v1, navigation only.** Finding/scanning surfaces (the candidate
...
```
  lachesis-ui  ──spawns──▶  python -m lachesis.nav.mcp_server <graph.kuzu>
   (this repo)   (stdio        (the engine: builds & serves the code graph,
...
### One command (pulls + builds the whole stack)
...
`python3` (3.10+), and Go 1.22+ to build the binary.
...
| `→` / `enter` | open — descend a folder, or jump into a symbol's neighborhood |
| `←` | go up a level |
| `1` / `o` | Overview — subsystems, entry points, the spine |
| `2` / `t` | Tree — source tree + per-file symbol outline |
| `3` | Neighborhood — the last symbol you opened |
| `tab` | switch panes in the neighborhood (reached-by ↔ uses) |
...
- **Overview** — the map. Subsystems ranked by how much of the graph's spine
  sits inside them, the entry points execution gets in through, and the
  highest-degree nodes as a cold-start "start here".
- **Tree** — the source tree with, alongside it, the symbol outline of the
  file under your cursor (what it *defines*, not its lines).
- **Neighborhood** — a symbol in focus with a source peek, and its callers /
  callees grouped by module (`http · 22`, `ftp · 21`, …) so a 150-caller hub
  reads as a distribution across the codebase, never a flat wall.
...
**Engine discovery** order: `$LACHESIS_PYTHON` → `~/.lachesis/venv/bin/python`
...
AGPL-3.0-or-later, matching the lachesis engine.
